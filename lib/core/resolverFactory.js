...
function getConstructor(source, config, registryClient) {
    ...
    if (/^git(\+(ssh|https?))?:\/\//i.test(source) || /\.git\/?$/i.test(source) || /^git@/i.test(source)) {
        source = source.replace(/^git\+/, '').replace('git://','https://');
	...
	// HG case: hg+ssh, hg+http, hg+https
    if (/^hg\+(ssh|https?):\/\//i.test(source)) {
        source = source.replace(/^hg\+/, '');
		return Q.fcall(function () {
            return [resolvers.Hg, source];
        });
    }
    ...
    if (/^\.\.?[\/\\]/.test(source) || /^~\//.test(source) || path.normalize(source).replace(/[\/\\]+$/, '') === absolutePath) {
		...
		// If not, check if source is a valid Mercurial repository
        .fail(function () {
            return Q.nfcall(fs.stat, path.join(absolutePath, '.hg'))
            .then(function (stats) {
                if (stats.isDirectory()) {
                    return function () {
                        return Q.resolve([resolvers.Hg, absolutePath]);
                    };
                }
